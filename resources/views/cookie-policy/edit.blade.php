<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('Edit Cookie Policy Content') }}</h1>
            <p class="text-gray-600 mt-2">{{ __('Update the content for the Cookie Policy page. All fields are editable from the database.') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('cookie-policy.update') }}" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center justify-end gap-4">
                <a href="{{ route('cookie-policy') }}" class="px-6 py-2.5 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-semibold transition-colors">
                    {{ __('Cancel') }}
                </a>
