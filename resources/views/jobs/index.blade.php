...
<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <form method="GET" action="{{ route('jobs.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-3">
                ...
                <div class="md:col-span-1">
                    <button class="w-full bg-primary hover:bg-primary-dark text-white font-semibold rounded-lg px-4 py-3">{{ __('Search') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-12 gap-8">
        <aside class="md:col-span-3 space-y-6">
            <div>
                <div class="flex items-center justify-between mb-3">
                    ...
                    <a href="{{ route('jobs.index') }}" class="text-xs text-primary">{{ __('Clear All') }}</a>
                </div>
                <div class="space-y-4 bg-white rounded-xl p-4 shadow-sm">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="remote" value="1" form="filters" @checked(request('remote')) class="rounded border-gray-300 text-primary focus:ring-primary">
            ...
            <div class="bg-primary-light p-4 rounded-xl border border-primary/20">
                <p class="text-sm font-semibold text-primary-dark">{{ __('Upload your resume') }}</p>
                <p class="text-xs text-primary mt-1">{{ __('We\'ll match you with the best jobs.') }}</p>
                <a href="{{ route('register') }}" class="inline-flex mt-3 px-3 py-2 bg-primary text-white rounded-lg text-sm">{{ __('Get started') }}</a>
            </div>
            ...
            <div class="bg-white rounded-xl p-4 shadow-sm">
                ...
                <div class="mt-3 space-y-2">
                    @foreach($categories as $category)
                        <a class="block text-sm text-gray-700 hover:text-primary" href="{{ route('jobs.index', array_merge(request()->except('page'), ['category' => $category->slug])) }}">{{ $category->name }}</a>
                    @endforeach
                </div>
            </div>
        </aside>
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center justify-between">
                                <div>
                                    <a href="{{ route('jobs.show', $job) }}" class="text-lg font-semibold text-gray-900 hover:text-primary-dark">{{ $job->title }}</a>
        ...
        <aside class="md:col-span-3 space-y-6">
            <div class="bg-white rounded-xl p-4 shadow-sm">
                <p class="text-sm font-semibold text-gray-800">{{ __('Be the first to see new jobs in') }} <span class="text-primary">{{ request('location') ?: __('your city') }}</span></p>
                <form class="mt-3 flex gap-2">
                    <input type="email" placeholder="{{ __('Email') }}" class="flex-1 rounded-lg border-gray-300" />
                    <button type="button" class="px-4 py-2 bg-primary text-white rounded-lg">{{ __('Subscribe') }}</button>
                </form>
            </div>
                ...
                <ul class="mt-3 space-y-3">
                    @foreach($popularCompanies as $company)
                        <li class="flex items-center justify-between text-sm">
                            <a href="{{ route('companies.show', $company) }}" class="text-gray-700 hover:text-primary">{{ $company->name }}</a>
