...
<x-app-layout>
    ...
    <div class="section-container py-10">
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
            <a href="{{ route('admin.sleepwell.home-items.export', ['screen' => $selectedScreen]) }}"
            ...
            <details class="rounded-lg border border-slate-200 p-2 text-xs dark:border-slate-700">
                ...
                <form method="POST" action="{{ route('admin.sleepwell.home-items.import') }}" class="mt-2">
                    @csrf
                    <textarea name="items_json" rows="6" class="w-full rounded-xl border-slate-300 text-xs dark:border-slate-700 dark:bg-slate-900" placeholder='[{"section_key":"featured_content","title":"Spring Forward"}]'></textarea>
